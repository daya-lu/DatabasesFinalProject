...
CREATE PROCEDURE Q2(IN s VARCHAR(1), eve VARCHAR(128), spo VARCHAR(64), seas VARCHAR(1))
BEGIN
...
INNER JOIN (
  ...
  FROM (
    ...
    FROM (
      ...
      AND eventID IN (SELECT eventID FROM Events WHERE event = eve AND sport = spo AND season = seas)
      GROUP BY athleteName, year, NOC
      ) A
...
CREATE PROCEDURE Q3(IN s VARCHAR(1), cname VARCHAR(32), seas VARCHAR(1))
BEGIN
...
CREATE PROCEDURE Q4(IN spo VARCHAR(32), cname VARCHAR(32), seas VARCHAR(1))
BEGIN
...
FROM (
  ...
  INNER JOIN (
    ...
    FROM Athletes 
    WHERE eventID IN (
      SELECT eventID FROM Events WHERE sport = spo AND season = seas) 
    AND NOC IN (
